...
/* USART2 液位传感�?ReceiveToIdle DMA 接收缓冲区�?*/
...
static void UART_Comm_ProcessLevelRxRange(uint16_t start, uint16_t end)
{
  ...
  {
    /*
     * 缺液控制板每秒主动上�?1 字节�?1010xxx�?     * �?3 位中 1 表示有液�? 表示缺液�?     *   bit0/K1 = 药液 1
     *   bit1/K2 = 药液 2
     *   bit2/K3 = 清洁�?     * 状态机负责校验�?5 位并转换成内部缺液位图�?     */
...
uint8_t UART_Comm_BuildStatusFrame(uint8_t *frame)
{
  ...
  /*
   * 状态帧由通信层统一填帧头、链路和校验�?   * 业务数据�?Base_BuildStatusData() 填入 frame[16]-frame[27]�?   */
